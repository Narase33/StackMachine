		...
		SECTION(expression) {
			try {
				...
				for (const auto& expec : expected) {
					REQUIRE(dataStack.top().getValue().getInt() == expec.getInt());
					dataStack.pop();
				}
...
TEST_CASE("ControlFlow-Test") {
	...
	// Complex all
	test_dataStack("1 + if ( 1 + 4 == 2 + 3 ) { 2 } else { 3 } + 4", { base::BasicType(7) });
	test_dataStack("1 + if ( 1 + 4 == 22 + 33 ) { 2 } else { 3 } + 4", { base::BasicType(8) });
