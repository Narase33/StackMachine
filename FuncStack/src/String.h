	...
	class String {
		public:
			...
			String& operator=(const String& str) {
				if (this != &str) this->_data.assign(str._data);
			...
			String& operator=(String&& str) noexcept {
				if (this != &str) this->_data.assign(std::move(str._data));
			...
			String& assign(const String& str) {
				if (this != &str) this->_data = str._data;
				return *this;
			}

			String& assign(String&& str) noexcept {
				if (this != &str) this->_data = std::move(str._data);
				return *this;
			}
			 ...
			 */
			std::pair<String, String> divide(const String& delimiter, size_type index) const;
...
String String::to_hexDump() const
{
	std::ostringstream stream;
	stream << std::hex << static_cast<int>(_data[0]);

	for (size_type i = 1; i < _data.length(); i++) {
		stream << " " << std::hex << static_cast<int>(_data[i]);
	}
